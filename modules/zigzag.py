...
def zigzag_order_function(bloc):
    """
    Réorganise les coefficients du bloc en zig-zag.
    
    Paramètres:
        bloc (numpy.ndarray): Bloc 8x8 quantifié.
        
    Retour:
        list: Vecteur 1x64 en zig-zag.
    """
    return [bloc.flatten()[i] for i in ZIGZAG_ORDER]

def inverse_zigzag_order(vector):
    """
    Reconstruit le bloc 8x8 à partir des coefficients en zig-zag.
    
    Paramètres:
        vector (list): Vecteur 1x64 en  zig-zag.
